import { 
  ...
  const TracePemotongan: FunctionComponent<TracePemotonganProps> = ({ data }) => {  
    ...
    return (
      ...
      >
        <HStack>
          <Text color='white' fontSize='xl' fontWeight='bold'>Pemotongan</Text>
        ...
        <UnorderedList color='white' spacing={'10px'} mt='10px'>
          ...
          <ListItem>Tanggal Pemotongan: {convertDate(data[3])}</ListItem>
          <ListItem>
            <HStack>
              <Text>Halal:</Text>
          ...
          <ListItem>Tanggal Input: {convertUnixDate(data[5])}</ListItem>
        </UnorderedList>
      </Flex>
    )
  }
